...
(() => {
  ...
  function techniqueGroupsMarkup() {
    return `<div class="technique-groups">${GROUPS.map(([id,title,copy]) => {
      ...
      return `<details class="technique-group" ${id === "prepare" ? "open" : ""}><summary><span class="technique-group-head"><span class="technique-group-title">${safe(title)}</span><span class="technique-group-copy">${safe(copy)} · ${items.length} techniques</span></span></summary><div class="technique-group-body"><div class="technique-grid">${items.map(techniqueCardMarkup).join("")}</div></div></details>`;
    }).join("")}</div>`;
  }
