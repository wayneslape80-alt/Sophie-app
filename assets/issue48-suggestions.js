...
(() => {
  ...
  function setSuggestionCredential(value) {
    try {
      if (value) localStorage.setItem(DEVICE_STORAGE_KEY, value);
  ...
  function itemById(id, parent = false) {
    return (parent ? state.parentItems : state.items).find(item => item.id === String(id)) || null;
  }
  ...
  function ensureDialogs() {
    ...
    host.innerHTML = `
      <dialog id="issue48-suggestion-dialog" class="issue48-fullscreen-dialog"><form class="dialog-body" id="issue48-suggestion-form"></form></dialog>
      ...
      <dialog id="issue48-detail-dialog" class="issue48-fullscreen-dialog"><div class="dialog-body" id="issue48-detail-body"></div></dialog>
      <dialog id="issue48-confirm-dialog" class="issue48-fullscreen-dialog"><div class="dialog-body" id="issue48-confirm-body"></div></dialog>
      <dialog id="issue48-parent-review-dialog" class="issue48-fullscreen-dialog"><form class="dialog-body" id="issue48-parent-review-form"></form></dialog>`;
  ...
  function domainEntry(domain) {
    ...
    return `${deviceStateMarkup()}<div class="issue48-actions" aria-label="Suggestions"><button class="secondary-button" type="button" data-issue48-open="${domain}" ${!suggestionCredential() ? "disabled" : ""}>${label}</button><button class="text-button" type="button" data-issue48-my>My suggestions</button></div>`;
  }

  function injectOpportunityActions() {
    const filters = document.getElementById("opportunity-filters");
    if (!filters) return;
    document.querySelectorAll("#view-opportunities .issue48-opportunity-actions").forEach(node => node.remove());
    const box = document.createElement("div");
    box.className = "issue48-opportunity-actions";
    ...
    else box.innerHTML = `${deviceStateMarkup()}<div class="issue48-actions" aria-label="Suggestions"><button class="secondary-button" type="button" data-issue48-open="contribute" ${!suggestionCredential() ? "disabled" : ""}>Suggest a contribution</button><button class="secondary-button" type="button" data-issue48-open="earn" ${!suggestionCredential() ? "disabled" : ""}>Suggest a paid job</button><button class="text-button" type="button" data-issue48-my>My suggestions</button></div>`;
    filters.insertAdjacentElement("afterend", box);
  }

  function injectSkillsActions() {
    const host = document.getElementById("skills-workspace");
    if (!host || host.querySelector(".issue48-skills-actions")) return;
    const box = document.createElement("section");
    box.className = "skills-section issue48-skills-actions";
    box.innerHTML = domainEntry("learn");
    host.prepend(box);
  }
  ...
  function formConfig(domain) {
    return {
      contribute: {
        ...
        intro: "Have an idea for something you could help with? Send it for a parent to look at.",
        footer: "A parent will work out the final plan with you before anything is added to Contribute.",
        fields: [
          ["title", "What would you like to help with?", "text", true, ""],
          ["description", "Anything else about the idea?", "textarea", false, ""],
          ["timingOrFrequency", "When or how often?", "text", false, "For example: once a week, after dinner, or when needed."]
        ]
      },
      earn: {
        title: "Suggest a paid job",
        intro: "Suggest extra work you'd be willing to do for pay. Normal responsibilities stay in Contribute.",
        footer: "Sending this doesn't create a paid job yet.",
        fields: [
          ["title", "What paid job are you suggesting?", "text", true, ""],
          ["description", "What would you do?", "textarea", true, ""],
          ["suggestedAmount", "What do you think it's worth?", "money", false, "Just a suggestion - a parent decides the agreed pay."]
        ]
      },
      learn: {
        title: "Suggest something to learn",
        intro: "What would you like to learn or get better at?",
        footer: "This saves your idea. A parent will check how it fits with Skills and safety before anything is added to Learn.",
        fields: [
          ["title", "What do you want to learn or practise?", "text", true, ""],
          ["interestReason", "Why does this interest you?", "textarea", false, ""],
          ["suggestedLink", "Link or idea", "url", false, "You can add a recipe, video or page you found."]
        ]
      }
    }[domain];
  }
  ...
  function fieldMarkup([name, label, kind, required, helper], value = "") {
    const id = `issue48-${name}`;
    const optional = required ? "" : ` <span style="font-weight:500">(Optional)</span>`;
    let control = "";
    if (kind === "textarea") control = `<textarea id="${id}" name="${name}" maxlength="1500" ${required ? "required" : ""}>${safe(value)}</textarea>`;
    else if (kind === "money") control = `<input id="${id}" name="${name}" type="number" min="0.01" step="0.01" inputmode="decimal" value="${value ? safe(value) : ""}">`;
    else control = `<input id="${id}" name="${name}" type="${kind === "url" ? "url" : "text"}" maxlength="${kind === "url" ? "2048" : "240"}" value="${safe(value)}" ${required ? "required" : ""}>`;
  ...
  function openSuggestionForm(domain, item = null) {
    const config = formConfig(domain);
    if (!config) return;
    ...
    state.createRequestId = item ? "" : requestId("suggestion-create");
  ...
  function payloadFromForm() {
    ...
    if (domain === "contribute") {
      ...
      payload.description = String(data.get("description") || "").trim();
      const amount = Number(data.get("suggestedAmount"));
      if (amount > 0) payload.suggestedAmount = amount;
    } else if (domain === "learn") {
      payload.interestReason = String(data.get("interestReason") || "").trim();
      const link = String(data.get("suggestedLink") || "").trim();
      if (link) {
  ...
  function setFormError(message, retry = false) {
    const host = document.getElementById("issue48-form-error");
    if (!host) return;
    host.innerHTML = message ? `<div class="issue48-error" role="status">${safe(message)}${retry ? `<div class="form-actions"><button type="button" class="secondary-button" data-issue48-retry-submit>Try again</button></div>` : ""}</div>` : "";
  }
  ...
  async function saveSuggestion(event) {
    ...
    try {
      ...
      state.loaded = true;
      state.formDirty = false;
      document.getElementById("issue48-suggestion-dialog").close();
      if (state.currentId) {
        toast("Suggestion updated");
        openSuggestionDetail(authoritative.id);
      } else {
  ...
  function closeSuggestionForm() {
    const dialog = document.getElementById("issue48-suggestion-dialog");
    if (!dialog?.open) return;
    if (!state.formDirty) return dialog.close();
    const body = document.getElementById("issue48-confirm-body");
    body.innerHTML = `<div class="dialog-head"><h2>Discard this suggestion?</h2><button class="close-button" type="button" data-issue48-close="issue48-confirm-dialog" aria-label="Close">×</button></div><p>Your unsent text is still in the form.</p><div class="form-actions"><button class="secondary-button" type="button" data-issue48-close="issue48-confirm-dialog">Keep editing</button><button class="primary-button" type="button" data-issue48-discard-form>Discard</button></div>`;
    document.getElementById("issue48-confirm-dialog").showModal();
  }

  function discardSuggestionForm() {
    state.formDirty = false;
  ...
  function renderMySuggestions() {
    const host = document.getElementById("issue48-suggestions-body");
    if (!host) return;
    if (!suggestionCredential()) {
      host.innerHTML = deviceStateMarkup();
      return;
    }
    ...
    const reviewed = state.items.filter(item => item.status !== "pending");
  ...
  async function openMySuggestions() {
    const dialog = document.getElementById("issue48-suggestions-dialog");
    if (!dialog.open) dialog.showModal();
    renderMySuggestions();
    await loadSuggestions({ quiet: true });
  }

  function sourceRows(item, parent = false) {
    const rows = [[parent ? "Title" : "What you suggested", item.title]];
    if (item.description) rows.push([item.domain === "earn" ? "What you would do" : "Details", item.description]);
    if (item.timingOrFrequency) rows.push(["When or how often", item.timingOrFrequency]);
    if (item.interestReason) rows.push(["Why it interests you", item.interestReason]);
    if (item.domain === "earn" && item.suggestedAmount > 0) rows.push([parent ? "Sophie suggested" : "You suggested", money(item.suggestedAmount)]);
    return rows;
  }

  function rowsMarkup(rows) {
    return `<dl>${rows.map(([label, value]) => `<dt>${safe(label)}</dt><dd>${safe(value)}</dd>`).join("")}</dl>`;
  }

  function linkMarkup(item) {
    const url = safeUrl(item.suggestedLink);
    return url && /^https?:/i.test(url) ? `<p><a class="product-link" href="${safe(url)}" target="_blank" rel="noopener noreferrer">Open suggested link</a></p>` : "";
  }

  function finalRows(item) {
    const rows = [];
    if (item.finalTitle) rows.push(["Title", item.finalTitle]);
    if (item.finalScope) rows.push(["Plan", item.finalScope]);
    if (item.finalRequiredness) rows.push(["How it fits", item.finalRequiredness === "expected" ? "Part of normal responsibilities" : item.finalRequiredness === "negotiated" ? "Agreed together" : item.finalRequiredness]);
    if (item.finalFrequency) rows.push(["When or how often", item.finalFrequency]);
    if (item.finalCompletionStandard) rows.push(["What done looks like", item.finalCompletionStandard]);
    if (item.finalEstimatedMinutes) rows.push(["About how long", `${item.finalEstimatedMinutes} minutes`]);
    if (item.domain === "earn" && item.finalAgreedPay > 0) rows.push(["Agreed pay", money(item.finalAgreedPay)]);
    if (item.domain === "learn") {
      if (item.finalSkillArea) rows.push(["Skill area", item.finalSkillArea]);
      if (item.finalTechniqueTitle) rows.push(["Technique", item.finalTechniqueTitle]);
      if (item.finalActivityTitle) rows.push(["Learning activity", item.finalActivityTitle]);
    }
    return rows;
  }

  function outcomeCopy(item) {
    if (item.status === "approved" && item.domain === "earn") return "It's now available in Earn. You still choose whether to take it.";
  ...
  function detailMarkup(item, { parent = false } = {}) {
    const pending = item.status === "pending";
    const final = finalRows(item);
    const support = pending ? (item.domain === "learn" ? "A parent will work out how this idea fits with Skills and safety." : "A parent can approve a final version, make changes before approving it, or decide not to use it this time.") : outcomeCopy(item);
    return `<div class="issue48-detail-grid"><section class="issue48-source"><h3>${parent ? "Sophie's suggestion" : "What you suggested"}</h3>${rowsMarkup(sourceRows(item, parent))}${linkMarkup(item)}</section>${!pending ? `<section class="issue48-final"><h3>${safe(statusCopy(item))}</h3>${item.reviewReasonText ? `<p>${safe(item.reviewReasonText)}</p>` : ""}${support ? `<p>${safe(support)}</p>` : ""}</section>` : ""}${final.length ? `<section class="issue48-final"><h3>Final plan</h3>${rowsMarkup(final)}</section>` : ""}</div>${pending && support ? `<p class="issue48-form-note">${safe(support)}</p>` : ""}`;
  }

  function openSuggestionDetail(id) {
    const item = itemById(id);
    if (!item) return;
    const body = document.getElementById("issue48-detail-body");
    body.innerHTML = `<div class="dialog-head"><div><span class="job-domain">${DOMAIN_LABELS[item.domain]}</span><h2>${item.status === "pending" ? "Your suggestion" : statusCopy(item)}</h2></div><button class="close-button" type="button" data-issue48-close="issue48-detail-dialog" aria-label="Close">×</button></div>${item.status === "pending" ? `<span class="issue48-status">Waiting for parent review</span>` : ""}${detailMarkup(item)}${item.status === "pending" ? `<div class="form-actions"><button class="secondary-button" type="button" data-issue48-edit="${safe(item.id)}">Edit suggestion</button><button class="secondary-button" type="button" data-issue48-withdraw="${safe(item.id)}">Withdraw suggestion</button></div>` : ""}`;
    document.getElementById("issue48-detail-dialog").showModal();
  }
  ...
  async function withdrawSuggestion(id, button) {
    ...
    try {
      ...
      if (index >= 0) state.items[index] = authoritative;
      document.getElementById("issue48-confirm-dialog").close();
      document.getElementById("issue48-detail-dialog")?.close();
      renderMySuggestions();
      toast("Withdrawn");
    } catch (error) {
  ...
  function parentSuggestionCard(item) {
    const action = item.status === "pending"
      ? `<button class="primary-button" type="button" data-issue48-parent-review="${safe(item.id)}">Review</button>`
      : `<button class="secondary-button" type="button" data-issue48-parent-view="${safe(item.id)}">View outcome</button>`;
  ...
  function renderParentSuggestions() {
    const anchor = document.getElementById("parent-goal-reviews");
    if (!anchor) return;
    let section = document.getElementById("issue48-parent-suggestions");
    if (!section) {
      section = document.createElement("section");
      section.id = "issue48-parent-suggestions";
      section.className = "issue48-parent-section";
      anchor.insertAdjacentElement("afterend", section);
    }
    const access = suggestionCredential()
      ? `<span><strong>Suggestions are ready on this device.</strong></span><button class="secondary-button" type="button" data-issue48-provision="rotate">Rotate Suggestions access</button>`
      : `<span><strong>Suggestions aren't available on this device yet.</strong><br>Set up Suggestions for Sophie's device.</span><button class="secondary-button" type="button" data-issue48-provision="provision">Set up Suggestions on this device</button>`;
    const pending = state.parentItems.filter(item => item.status === "pending");
    const reviewed = state.parentItems.filter(item => item.status !== "pending").slice(0, 8);
    section.innerHTML = `<div class="issue48-parent-head"><div><h3>Sophie suggestions</h3><p>Ideas Sophie has sent for review. Nothing becomes active until you approve a final version.</p></div></div><div class="issue48-parent-access">${access}</div>${state.parentUnavailable ? `<div class="issue48-error" role="status">Suggestions couldn't be refreshed. Reload before reviewing anything.</div>` : ""}<h4>Waiting for review</h4><div class="issue48-list">${state.parentLoading && !state.parentLoaded ? `<div class="skeleton"></div>` : pending.length ? pending.map(parentSuggestionCard).join("") : `<div class="empty"><strong>No suggestions waiting</strong></div>`}</div><h4 style="margin-top:18px">Recently reviewed</h4><div class="issue48-list">${reviewed.length ? reviewed.map(parentSuggestionCard).join("") : `<div class="empty"><strong>No reviewed suggestions yet</strong></div>`}</div>`;
  }

  async function loadParentSuggestions({ quiet = false } = {}) {
    if (!parentSessionValid()) return [];
    if (navigator.onLine === false) {
    ...
    try {
      ...
      state.parentUnavailable = true;
      if (!quiet) toast("Sophie suggestions could not be loaded.");
      return state.parentItems;
    } finally {
  ...
  async function provisionSuggestionDevice(mode, button) {
    ...
    try {
      ...
      state.items = [];
      renderParentSuggestions();
      injectOpportunityActions();
      injectSkillsActions();
      toast("Suggestions are ready on this device.");
    } catch (error) {
      toast(error.message || "Suggestions could not be set up on this device.");
    } finally {
      button.disabled = false;
    }
  }

  function reviewReasonOptions(domain) {
    if (domain === "contribute") return ["Not right now", "Needs a different plan", "We already have this covered", "Something else"];
    if (domain === "earn") return ["Not right now", "We don't need this paid job right now", "This is part of normal responsibilities", "Needs a different plan", "Something else"];
    return ["Not right now", "Needs a different safety setup", "There's something useful to learn first", "We don't have the equipment/time/support for this yet", "Something else"];
  }
  ...
  function openParentReview(id) {
    ...
    const preset = document.getElementById("issue48-review-reason-preset");
    const reason = document.getElementById("issue48-review-reason");
    preset.addEventListener("change", () => {
      if (preset.value !== "Something else") reason.value = preset.value;
      else reason.focus();
    });
    document.getElementById("issue48-parent-review-dialog").showModal();
  }

  function openParentOutcome(id) {
    const item = itemById(id, true);
    if (!item || item.status === "pending") return;
    const form = document.getElementById("issue48-parent-review-form");
    form.innerHTML = `<div class="dialog-head"><div><span class="job-domain">${DOMAIN_LABELS[item.domain]}</span><h2>${safe(statusCopy(item))}</h2></div><button class="close-button" type="button" data-issue48-close="issue48-parent-review-dialog" aria-label="Close">×</button></div>${detailMarkup(item, { parent: true })}<div class="form-actions"><button class="secondary-button" type="button" data-issue48-close="issue48-parent-review-dialog">Close</button></div>`;
    document.getElementById("issue48-parent-review-dialog").showModal();
  }

  function parentReviewPayload(item, form, outcome) {
    const data = new FormData(form);
    const reviewReasonText = String(data.get("reviewReasonText") || "").trim();
    if (["declined", "not_yet"].includes(outcome) && !reviewReasonText) throw new Error("Add a direct factual reason for this decision.");
    const preset = document.getElementById("issue48-review-reason-preset")?.value || "";
    const payload = {
    ...
    if (item.domain === "contribute") {
      ...
      payload.finalRequiredness = String(data.get("finalRequiredness") || "");
      payload.finalFrequency = String(data.get("finalFrequency") || "").trim();
      payload.finalCompletionStandard = String(data.get("finalCompletionStandard") || "").trim();
    } else if (item.domain === "earn") {
      payload.finalTitle = String(data.get("finalTitle") || "").trim();
      payload.finalScope = String(data.get("finalScope") || "").trim();
      payload.finalCompletionStandard = String(data.get("finalCompletionStandard") || "").trim();
      ...
      payload.mappedCandidateId = mapping.candidateId;
      payload.mappedTechniqueId = mapping.techniqueId;
    }
    return payload;
  }

  function parentReviewError(message) {
    const host = document.getElementById("issue48-parent-review-error");
    if (host) host.innerHTML = message ? `<div class="issue48-error" role="status">${safe(message)}</div>` : "";
  }

  async function saveParentReview(event) {
    event.preventDefault();
    const form = event.currentTarget;
  ...
  async function retryDeviceState() {
    if (!suggestionCredential()) {
      injectOpportunityActions();
      injectSkillsActions();
      renderMySuggestions();
      return toast("Suggestions aren't available on this device yet. Ask a parent to set up Suggestions on this device.");
    }
    await loadSuggestions({ quiet: true });
    injectOpportunityActions();
    injectSkillsActions();
  ...
  const baseRenderJobsIssue48 = renderJobs;
  renderJobs = function(...args) {
    const result = baseRenderJobsIssue48(...args);
    injectOpportunityActions();
    return result;
  };

  const baseRenderSkillsIssue48 = renderSkills;
  renderSkills = function(...args) {
    const result = baseRenderSkillsIssue48(...args);
    injectSkillsActions();
    return result;
  };

  const baseRenderParentIssue48 = renderParent;
  renderParent = function(...args) {
    const result = baseRenderParentIssue48(...args);
    renderParentSuggestions();
    void loadParentSuggestions({ quiet: true });
    return result;
  };

  const baseLockParentIssue48 = lockParentMode;
  lockParentMode = function(...args) {
    state.parentItems = [];
  ...
  document.addEventListener("click", event => {
    const open = event.target.closest("[data-issue48-open]");
    if (open) { event.preventDefault(); openSuggestionForm(open.dataset.issue48Open); return; }
    const mine = event.target.closest("[data-issue48-my]");
    if (mine) { event.preventDefault(); closeDialog("issue48-confirm-dialog"); void openMySuggestions(); return; }
    const view = event.target.closest("[data-issue48-view]");
    if (view) { event.preventDefault(); openSuggestionDetail(view.dataset.issue48View); return; }
    const edit = event.target.closest("[data-issue48-edit]");
    if (edit) { event.preventDefault(); const item = itemById(edit.dataset.issue48Edit); if (item?.status === "pending") { closeDialog("issue48-detail-dialog"); openSuggestionForm(item.domain, item); } return; }
    const withdraw = event.target.closest("[data-issue48-withdraw]");
    if (withdraw) { event.preventDefault(); confirmWithdraw(withdraw.dataset.issue48Withdraw); return; }
    const confirm = event.target.closest("[data-issue48-confirm-withdraw]");
    if (confirm) { event.preventDefault(); void withdrawSuggestion(confirm.dataset.issue48ConfirmWithdraw, confirm); return; }
    const discard = event.target.closest("[data-issue48-discard-form]");
    if (discard) { event.preventDefault(); discardSuggestionForm(); return; }
    const retry = event.target.closest("[data-issue48-device-retry]");
    if (retry) { event.preventDefault(); void retryDeviceState(); return; }
    const retrySubmit = event.target.closest("[data-issue48-retry-submit]");
    if (retrySubmit) { event.preventDefault(); document.getElementById("issue48-suggestion-form")?.requestSubmit(); return; }
    const cancel = event.target.closest("[data-issue48-cancel-form]");
    if (cancel) { event.preventDefault(); closeSuggestionForm(); return; }
    const close = event.target.closest("[data-issue48-close]");
    if (close) { event.preventDefault(); closeDialog(close.dataset.issue48Close); return; }
    const review = event.target.closest("[data-issue48-parent-review]");
    if (review) { event.preventDefault(); openParentReview(review.dataset.issue48ParentReview); return; }
    const parentView = event.target.closest("[data-issue48-parent-view]");
    if (parentView) { event.preventDefault(); openParentOutcome(parentView.dataset.issue48ParentView); return; }
    const provision = event.target.closest("[data-issue48-provision]");
    if (provision) { event.preventDefault(); void provisionSuggestionDevice(provision.dataset.issue48Provision, provision); }
  });

  window.addEventListener("online", () => {
    injectOpportunityActions();
    injectSkillsActions();
    if (document.getElementById("issue48-suggestions-dialog")?.open) void loadSuggestions({ quiet: true });
    if (parentSessionValid() && document.getElementById("parent-dialog")?.open) void loadParentSuggestions({ quiet: true });
  });
  window.addEventListener("offline", () => {
    injectOpportunityActions();
    injectSkillsActions();
    if (document.getElementById("issue48-suggestions-dialog")?.open) renderMySuggestions();
  });
