...
new_group_markup = 'return `<details class="technique-group" ${id === "prepare" ? "open" : ""}><summary><span class="technique-group-head"><span class="technique-group-title">${safe(title)}</span><span class="technique-group-copy">${safe(copy)} · ${items.length} techniques</span></span></summary><div class="technique-group-body"><div class="technique-grid">${items.map(techniqueCardMarkup).join("")}</div></div></details>`;'
...
new_safety_options = '${REC_SAFETY_OPTIONS.map(([value,label]) => `<button class="rec-choice" type="button" data-v27-technique-safety="${safe(value)}" aria-pressed="${flow.safetySupport === value}">${safe(label)}</button>`).join("")}'
...
new_support_options = '${REC_SUPPORT_OPTIONS.map(([value,label]) => `<button class="rec-choice" type="button" data-rec-support="${safe(value)}" aria-pressed="${app.rec.supportChoice === value}">${safe(label)}</button>`).join("")}'
...
replacements = [
    (
        ...
        '      body.innerHTML = `<div class="dialog-head"><div><span class="opportunity-detail-domain">COOKING · LEARN</span><h2>Set up this learning choice</h2></div><button type="button" class="close-button" data-rec-exit aria-label="Close">×</button></div><div class="rec-progress">'
    ),
    (
        '      title.textContent = "Choose a real activity";\n      body.innerHTML = `<div class="rec-screen">',
        '      body.innerHTML = `<div class="dialog-head"><div><span class="opportunity-detail-domain">COOKING · LEARN</span><h2>Choose a real activity</h2></div><button type="button" class="close-button" data-rec-exit aria-label="Close">×</button></div><div class="rec-progress">'
    ),
    (
        '      title.textContent = "Choose the support you want";\n      body.innerHTML = `<div class="rec-screen">',
        '      body.innerHTML = `<div class="dialog-head"><div><span class="opportunity-detail-domain">COOKING · LEARN</span><h2>Choose the support you want</h2></div><button type="button" class="close-button" data-rec-exit aria-label="Close">×</button></div><div class="rec-progress">'
    )
]
