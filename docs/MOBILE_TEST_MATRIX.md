...
The suite must separately emulate Android Chrome desktop-site/page-zoom behaviour where the browser continues exposing a `980px` layout viewport for a `360px` phone even after the viewport tag is repaired. In that condition, the app must:

- retain the compact single-column structure and five-item bottom navigation;
- publish an explicit effective-scale diagnostic;
- compensate for the layout-to-screen ratio so physical page, section, row and body text resolve to the approved type roles;
- preserve an effective 48px touch target and approximately 84px bottom navigation;
- avoid horizontal overflow.

This prevents a compact layout that is still globally shrunk from being accepted as a phone pass.
