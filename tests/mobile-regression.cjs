...
async function openDesktopSitePhone(browser, baseUrl) {
  const context = await browser.newContext({
    viewport: { width: 980, height: 1200 },
    screen: { width: 980, height: 1200 },
    deviceScaleFactor: 1,
    hasTouch: true,
    isMobile: false,
    serviceWorkers: "block",
  ...
  await context.addInitScript(() => {
    try {
      Object.defineProperty(window.screen, "width", { configurable: true, get: () => 360 });
      Object.defineProperty(window.screen, "height", { configurable: true, get: () => 800 });
    } catch {
      Object.defineProperty(Screen.prototype, "width", { configurable: true, get: () => 360 });
      Object.defineProperty(Screen.prototype, "height", { configurable: true, get: () => 800 });
    }
...
async function runDesktopSiteScaleTrap(browser, baseUrl) {
  const { context, page } = await openDesktopSitePhone(browser, baseUrl);
  await page.getByRole("button", { name: "Skills", exact: true }).click();
  await page.getByRole("heading", { name: "Currently learning" }).waitFor();
  const audit = await page.evaluate(() => {
    const root = document.documentElement;
    const scale = Number.parseFloat(root.dataset.effectiveMobileScale || "1");
    const size = selector => Number.parseFloat(getComputedStyle(document.querySelector(selector)).fontSize) / scale;
    const height = selector => document.querySelector(selector).getBoundingClientRect().height / scale;
    return {
      innerWidth,
      screenWidth: screen.width,
      layoutRatio: innerWidth / screen.width,
      compactDevice: root.classList.contains("compact-device"),
      effectiveScaleActive: root.classList.contains("effective-mobile-scale"),
      scale,
      rootFontSize: Number.parseFloat(getComputedStyle(root).fontSize) / scale,
      pageTitleSize: size("#skills-workspace > .page-intro h2"),
      sectionTitleSize: size("#skills-workspace .section-heading h2"),
      rowTitleSize: size("#skills-workspace .skills-learning-list .job-open h3"),
      rowBodySize: size("#skills-workspace .skills-learning-list .job-purpose"),
      actionHeight: height("#skills-workspace .skills-learning-list .primary-button, #skills-workspace .skills-learning-list .secondary-button"),
      navHeight: height(".bottom-nav"),
      navColumns: getComputedStyle(document.querySelector(".bottom-nav")).gridTemplateColumns.split(" ").length,
      domainColumns: getComputedStyle(document.querySelector(".learning-domain-grid")).gridTemplateColumns.split(" ").length,
      scrollWidth: root.scrollWidth
    };
  });
  assert(audit.innerWidth >= 900, `desktop-site trap did not expose a wide viewport (${audit.innerWidth}px)`);
  assert(audit.layoutRatio > 2.5, `desktop-site trap ratio is too small (${audit.layoutRatio})`);
  assert.strictEqual(audit.compactDevice, true, "desktop-site phone was not classified as compact");
  assert.strictEqual(audit.effectiveScaleActive, true, "effective mobile scale did not activate");
  assert(Math.abs(audit.scale - audit.layoutRatio) <= 0.02, `effective scale ${audit.scale} does not match layout ratio ${audit.layoutRatio}`);
  assert(audit.rootFontSize >= 15.9, `effective root text is too small (${audit.rootFontSize}px)`);
  assert(audit.pageTitleSize >= 24, `effective page title is too small (${audit.pageTitleSize}px)`);
  assert(audit.sectionTitleSize >= 20, `effective section title is too small (${audit.sectionTitleSize}px)`);
  assert(audit.rowTitleSize >= 16, `effective row title is too small (${audit.rowTitleSize}px)`);
  assert(audit.rowBodySize >= 14, `effective row body is too small (${audit.rowBodySize}px)`);
  assert(audit.actionHeight >= 47.5, `effective action target is too small (${audit.actionHeight}px)`);
  assert(audit.navHeight >= 83, `effective navigation is too short (${audit.navHeight}px)`);
  assert.strictEqual(audit.navColumns, 5, "desktop-site phone retained a navigation rail");
  assert.strictEqual(audit.domainColumns, 1, "desktop-site phone retained multiple domain columns");
  assert(audit.scrollWidth <= audit.innerWidth + 1, `desktop-site fallback overflows ${audit.scrollWidth} > ${audit.innerWidth}`);
  await context.close();
  return { desktopSiteViewportInjected: "PASS", effectiveMobileScale: "PASS", physicalTypeScale: "PASS", physicalTouchTargets: "PASS" };
}
...
(async () => {
  ...
  try {
    ...
    const desktopSiteScaleTrap = await runDesktopSiteScaleTrap(browser, baseUrl);
    ...
    console.log(JSON.stringify({ viewportResults, mobileViewportContract, scaledDesktopTrap, desktopSiteScaleTrap, androidTextScaling, functional, accessibility, pwa }, null, 2));
  } finally {
    await browser.close();
